...
- **Motherboard (základní deska):** hl. deska, na které jsou připojeny všechny komponenty
...
- **`GPU` Grafická karta:** zpracovává grafiku a vizuální výstupy
- **`PSU` Napájecí zdroj:** dodává energii všem komponentám

## Základní parametry

- **`GHz` Rychlost procesoru:** určuje, kolik operací může CPU vykonat za sekundu
- **`GB` Velikost RAM:** určuje, kolik dat může být současně uloženo pro rychlý přístup
- **`GB` `TB` Kapacita úložiště:** určuje, kolik dat může být trvale uloženo
- **`GB/s` Rychlost čtení/zápisu:** určuje, jak rychle může být data načítána nebo zapisována na úložiště
...
- **Kombinovaná zařízení:** kombinace (dotykové obrazovky)
