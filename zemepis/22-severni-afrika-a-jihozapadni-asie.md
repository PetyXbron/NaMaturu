---
title: Severní Afrika a jihozápadní Asie
description: Otázka 22 - Regionální geografie světa - severní Afrika a jihozápadní Asie
state: 1
---

# **Severní Afrika a jihozápadní Asie**

- Přírodní podmínky - geologický vývoj, základní geomorfologické celky, klimatické a hydrologické poměry.
- Sociálně ekonomické podmínky.
- Postavení makroregionu v globálním měřítku.

---

::: danger NEDOKONČENO
Na této stránce ještě stále pracujeme.
:::

## Přírodní podmínky

### Geologický vývoj

- Litosférické desky: Euroasijská, Africká, Arabská

### Geomorfologické celky

::: warning STÁLE UPRAVUJEME
Tato stránka je stále ve vývoji a může se brzy změnit.
:::

### Klimatické a hydrologické poměry

- Tropický pás
- V jihozápadní Asii jedny z nejvyšších teplot
- Téměř žádné srážky
- Řeky Nil, Eufrat a Tigris
- Šot (občasná jezera) a vádí (sezónní vodní toky)

### Půdy

::: warning STÁLE UPRAVUJEME
Tato stránka je stále ve vývoji a může se brzy změnit.
:::

## Sociálně ekonomické podmínky

### Politické a ekonomické poměry

| **Země**                      | **Státní zřízení**                    | **Měna** | **Hl. město** | **Liga arabských států**  | **OPEC** |
|-------------------------------|---------------------------------------|----------|---------------|:-------------------------:|:--------:|
| :egypt: Egypt                 | 🧑‍💼 <Tip id="ze3">poloprez. rep.</Tip> | EGP £    | Káhira        |            ✅            |    ❌    |
| :algeria: Alžírsko            | 🧑‍💼 <Tip id="ze3">poloprez. rep.</Tip> | DZD      | Alžír         |            ✅            |    ✅    |
| :morocco: Maroko              | 👑 <Tip id="ze6">konst. mon.</Tip>    | MAD      | Rabat         |            ✅            |    ❌    |
| :tunisia: Tunisko             | 🧑‍💼 <Tip id="ze2">prez. rep.</Tip>     | TND      | Tunis         |            ✅            |    ❌    |
| :libya: Libye                 | 🏛️ <Tip id="ze1">parl. rep.</Tip>     | LYD      | Tripolis      |            ✅            |    ✅    |
| :israel: Izrael               | 🏛️ <Tip id="ze1">parl. rep.</Tip>     | ILS ₪    | Jeruzalém     |            ❌            |    ❌    |
| :tr: Turecko                  | 🧑‍💼 <Tip id="ze2">prez. rep.</Tip>     | TRY      | Ankara        |            ❌            |    ❌    |
| :saudi_arabia: Saúdská Arábie | 👑 <Tip id="ze8">abs. mon.</Tip>      | SAR      | Rijád         |            ✅            |    ✅    |
| :qatar: Katar                 | 👑 <Tip id="ze8">abs. mon.</Tip>      | QAR      | Dauhá         |            ✅            |    ❌    |
| :iran: Írán                   | 🌙 <Tip id="ze12">isl. rep.</Tip>     | IRR      | Teherán       |            ❌            |    ✅    |
| :iraq: Irák                   | 🏛️ <Tip id="ze1">parl. rep.</Tip>     | IQD      | Bagdád        |            ✅            |    ✅    |
| :syria: Sýrie                 | 🧑‍💼 <Tip id="ze3">poloprez. rep.</Tip> | SYP £    | Damašek       |            ✅            |    ❌    |
| :yemen: Jemen                 | 🧑‍💼 <Tip id="ze2">prez. rep.</Tip>     | YER      | San'á         |            ✅            |    ❌    |

### Hospodářství

#### Suroviny

- Chemický průmysl, výroba textilu, potravinářský průmysl

#### Hlavní odvětví

- Izrael - brusírny diamantů
- Náboženský a rekreační cestovní ruch

## Postavení makroregionu v globálním měřítku

- Izrael a pásmo Gazy - v historii sem přišlo hodně Rusů

### Postavení jednotlivých zemí

| Demografický ukazatel      | Průměrná hodnota    | Hodnocení                     |
|---------------------------:|---------------------|-------------------------------|
| **Porodnost**              | 22 ‰                | <code class="cgreen">+</code> |
| **Úmrtnost**               | 5 ‰                 | <code class="cgreen">+</code> |
| **Přirozený přírůstek**    | 17 ‰                | <code class="cgreen">+</code> |
| **Úhrnná plodnost**        | 2,9                 | <code class="cgreen">+</code> |
| **Kojenecká úmrtnost**     | 18 ‰                | <code class="cred">--</code>  |
| **Naděje dožití**          | 73 let              | <code class="cred">-</code>   |
| **Věkové pyramidy**        | expanzivní          | <code class="cgreen">+</code> |
| **Migrační saldo**         | -1 mil.             | <code class="cred">-</code>   |

| Socioekonomický ukazatel   | Průměrná hodnota    | Hodnocení                     |
|---------------------------:|---------------------|-------------------------------|
| **PPP (HDP na obyv.)**     | 15 000 $            | <code class="cred">-</code>   |
| **HDI**                    | 0,72                | <code class="cred">-</code>   |
| **Gini index**             | 0,38                | <code class="cred">-</code>   |
| **Míra urbanizace**        | 65 %                | <code class="cred">-</code>   |
